...
/**
    ...
    */
export class CanvasPanel extends Panel implements IDrawable
{
    private imageW: number;
    private imageH: number;
    private bgColor: string;
    private bgOffset: IPoint = { x: 0, y: 0 };
    private bgStyle = BackgroundStyle.NO_REPEAT;
    /** @protected */
    _drawable: IDrawable;
    ...
    */
    constructor(w: number, h: number, drawable: IDrawable);
    constructor(w: number, h: number, imageOrDrawable?: any)
    {
        ...
        {
            this._drawable = new SpriteSheetImage(imageOrDrawable, 0, 0, imageOrDrawable.width, imageOrDrawable.height);
        }
        else
        {
    ...
    public update(dt: number): CanvasPanel { return this; }

    public setBackgroundColor(color: string): CanvasPanel
    {
        this.bgColor = color;
        return this;
    }
    ...
    public setBackgroundPosition(offset: number, offsetY?: number): CanvasPanel
    {
    ...
    public setBackgroundStyle(style: BackgroundStyle): CanvasPanel
    {
        this.bgStyle = style;
        return this;
    }
    public getBackgroundStyle(): BackgroundStyle
    {
        return this.bgStyle;
    }
    ...
    public draw(context: CanvasRenderingContext2D, x: number = this.x, y: number = this.y, w: number = this.w, h: number = this.h)
    {
        if (this.visible)
        {
            ...
            if (this._drawable)
            {
                ...
                switch (this.bgStyle)
                {
                    case BackgroundStyle.REPEAT:
                        this.tileBackgroundBoth(context, x, y, w, h);
                        break;
                    case BackgroundStyle.REPEAT_X:
                        this.tileBackgroundHorizontal(context, x, y, w);
                        break;
                    case BackgroundStyle.REPEAT_Y:
...
/**
    ...
    */
export class ScrollingPanel extends CanvasPanel
{
    /** X-velocity in ppms */
    public vx = 0;
    /** Y-velocity in ppms */
    ...
    /**
        ...
        */
    //constructor(x: number, y: number, w: number, h: number, drawable: Drawable);
    constructor(w: number, h: number, drawable: IDrawable, ppsX = 0, ppsY = 0)
    {
        super(w, h, drawable);
        this.setVelocity(ppsX, ppsY);
        this.setBackgroundStyle(BackgroundStyle.REPEAT);
    }
    ...
    * @override
    */
    public update(dt: number): ScrollingPanel
    {
